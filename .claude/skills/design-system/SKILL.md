...
- Every `ld` call is described beside its registration, behind the `meta` feature: `SIGNATURES` and the fields of the tables it takes in the group's `constants.rs`, turned into tealr's model in its `describe.rs`, collected by `ld/walker.rs`. A new call, option or key carries its description in the same change, and `./packaging/meta/update.sh` regenerates `meta/ld.lua`; `cargo test --features meta` checks both.
