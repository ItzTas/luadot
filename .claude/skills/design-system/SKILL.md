...
| `meta/` | `meta/ld.lua`, the lua-language-server definitions of `ld`, generated by `packaging/meta/update.sh` and embedded in the binary |
| `src/bin/` | binaries behind a cargo feature, never shipped: `luadot-meta` renders the definitions |
...
- `constants.rs` holds what two or more files of the module read. A value a single file uses lives in that file: a literal at the call site, or a `const` above it when the number needs a name.
...
- One `ld` function per file, one directory per group: `ld/root/` is `ld.<function>`, and a named group (`ld/git/`) owns its `NAMESPACE` and becomes `ld.<namespace>.<function>`. Each group has a `table.rs` listing its functions. A group of setters that only read a value and store it shares one file (`ld/opt/set.rs`), one function per option.
- Every `ld` call is described in the group's `describe.rs`, the only file behind the `meta` feature: `SIGNATURES`, the fields of the tables it takes and the typenames sit next to the tealr model they feed, collected by `ld/walker.rs`. A name the runtime also uses stays in `constants.rs` and is imported from there. A new call, option or key carries its description in the same change, and `./packaging/meta/update.sh` regenerates `meta/ld.lua`; `cargo test --features meta` checks both.
