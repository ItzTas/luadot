...
pub const PASSPHRASE_WARNING: &str = "passphrase mode is weaker than keys: one passphrase opens every secret, everyone sharing the repository shares it, and changing it means re-encrypting everything (silence this with `ld.crypt.passphrase_warn(false)`)";
