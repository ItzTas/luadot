...
Nothing here is implemented. This supersedes the ROADMAP item asking for a
`{{ var }}` mini-syntax for `ld.alt.render` — `<%= var %>` covers substitution
and adds control flow, so one form replaces both.

## The two forms

| Form | Shape | Produces | Has |
| --- | --- | --- | --- |
| Directory | `.zshrc.luadot/` with `luadot.lua` and `zshrc.tmpl.zsh` | any number of files | variants, several outputs, `dest`/`link`/`conflict`, `require` of `lua/` |
| File | `.zshrc.luadot` as a plain file | exactly one file, at the mirrored path | the `ld` interface and nothing else |

Both are resolved by `luadot alt`. `apply` and `status` walk past both, exactly
...
The tags are EJS's, with EJS's meanings. Nothing is redefined and nothing is
invented:
...
escapes they genuinely are the same tag.
...
- `<%_` removes **all** whitespace before the tag, newlines included.
- `-%>` removes the newline immediately after the tag.
- `_%>` removes **all** whitespace after the tag, newlines included.

In HTML that is harmless. In a `.zshrc` it is not, and this deserves to be said
...
`<%_` and `_%>` stay in because they are part of the standard and are opt-in;
`<%` plus `-%>` is the recipe to reach for.
...
`.luadot` **file** is a template rendered directly. One rule, two forms, and
`template_target`/`template_dir` work unchanged for both.
...
Rejected: a second suffix (`.lt`, `.esh`) — a new concept, new constants, new
resolution rules, for a form whose selling point is that there is less to learn.
...
Shared by both forms. Text in, Lua chunk out, run by mlua.

Literals never reach the generated source. They go into a `Vec<String>` on the
Rust side and are referenced by index, which removes every escaping question —
a dotfile holding `]==]`, quotes or invalid Lua is just bytes in a table.
...
2. **Generated lines are padded to match source lines.** Newlines inside
   literals do not exist in the generated source, so before each segment the
   compiler appends newlines until the generated line number equals the source
   line number. With `set_name(path)`, a runtime error in a template reports
   the real line of the real file. This is the difference between a usable
   template and an unusable one.
3. **The buffer lives on the Rust side**, an `Arc<Mutex<String>>` captured by
   the two closures (`Rc` is out — the `send` feature of mlua is on). Compared
...
4. **`__ld_write` refuses `nil`.** An undefined name in a template is `nil`,
   and `tostring(nil)` would put the word `nil` into the generated file and say
   nothing about it — a typo in `<%= edtior %>` producing `export EDITOR=nil`
   is exactly the silent corruption this document keeps refusing elsewhere. It
   errors instead, naming the line and the fact that the value was `nil`.
   Everything else goes through `tostring`, `false` included.

### Variables

A free name in a template — `editor` in `<%= editor %>` — is resolved against
the environment the chunk runs in: the `vars` table of the call first, then the
globals, through `__index`. That is the environment `ld.alt.render` already
builds, reused as it stands.

Which means the two forms differ here, and this is the sharpest difference
between them:
...
looking for the closing delimiter — about forty lines, and the failure it
prevents is a silent mis-parse.

A `<%#` tag is the exception: its body is prose, not Lua, so it is scanned for
the closing delimiter alone and produces no segment at all — neither a literal
nor a line of generated code, only the newline padding that keeps the following
lines aligned.
...
`__index = globals`, so `ld` stays reachable. It returns a string, so
`Output`, `Content::Text` and everything downstream of `alt` are untouched.

A separate function, not extension detection inside `render`: `render` runs a
Lua script that returns a string, `expand` runs a text file that emits one.
Two different contracts deserve two names.
...
already does for generated content — no code path is added there.
...
`ld.path.dir` stays `nil` — there is no template directory. `ld.path.repo`
...
- `target()` — the `Standalone` arm joins the `Template` arm; both strip the
  suffix through `template_target`.
- `collect_into` — the non-directory branch checks `is_template` before
  pushing `Entry::File`. Without it a standalone template would be copied to
  the home directory verbatim, tags and all.
- `collect_entries` — the single-file root case gets the same check.
- `collect_files` — `Standalone` pushes the file itself, so `rm` and `add`
  reach it by name.
...
today; it becomes `exists`, so `luadot alt ~/.zshrc` reaches a standalone
...
- `run.rs` — a chunk emitting nothing, a `return` mid-template, a runtime
  error, `<%= nil %>` and an undefined name both erroring, `<%= false %>`
  writing `false`.
...
what pays for the naming tradeoff, and because the third one constrains nothing
only as long as the format stays what it is.
...
nothing in the name says which. The answer costs nothing now that `<%#`
exists — a directive on the first line, ignored by the renderer:
...
back — which is most of what `svelte-language-server` and Volar are.

Crate, when the time comes: `lsp-server`, the rust-analyzer one — synchronous,
minimal, and it keeps tokio out of a binary that has no async in it.
`tower-lsp` had no release since August 2023; its live fork is
`tower-lsp-server`.

Order matters here: a language server is written against a settled syntax. The
first two layers are worth having as soon as the format exists, the third one
only after it has been used enough to stop changing.
...
and the tests, benchmarks and docs are not features.
...
- Whether a line-bounded trim is worth having beside the greedy pair. EJS's
  ...
  block to disappear cleanly, there is no spelling for it. The grammar leaves
  `<%|` and `<%~` unclaimed on the opening side, so one could be added without
  losing the highlighting; the closing side offers only `%>`, `-%>` and `_%>`,
  so it would have to stay asymmetric.
