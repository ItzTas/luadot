...
| `ld.alt.concat(sections, separator)` | a list of strings or sections, and an optional string | The sections joined into one string, in the order they are given, with the separator between them; a newline when none is given. |
...
`ld.alt.concat` joins what the other calls return, and drops the sections whose
`when` is `false`:
...
return ld.alt.concat({
  "# generated by luadot",
  ld.alt.read("aliases.zsh"),
  { content = ld.alt.read("laptop.zsh"), when = ld.class.get("form-factor") == "laptop" },
  ld.alt.render("prompt.tmpl.lua", { color = "cyan" }),
  ld.alt.expand("greeting.tmpl.zsh", { editor = "nvim" }),
})
```

| Key | Values | Effect |
| --- | --- | --- |
| `content` | a string | The text of the fragment, whatever produced it. Required. |
| `when` | `true`, `false` | Whether the section lands. Defaults to `true`, and only `false` leaves it out. |

A string in the list is a section carrying only `content`. Lua builds the
whole table before the call, so a `content` behind a `when = false` is built
and thrown away; keep an expensive one out with an `if` instead. A section
that is neither a string nor a table, one without a `content`, an unknown key
and a `when` that is not a boolean are all errors naming the section by its
position.

The second argument is what goes between the fragments, a newline when it is
left out. `""` suits fragments that already end in one:

```lua
return ld.alt.concat(parts, "")
```
